...
export class MarkdownParser {
    ...
    static parse(markdown: string): Block[] {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            // 3. Headings
            if (trimmed.startsWith('# ')) {
                blocks.push({ id: uuidv4(), type: 'heading_1', content: trimmed.substring(2).trim() });
                continue;
            }
            if (trimmed.startsWith('## ')) {
                blocks.push({ id: uuidv4(), type: 'heading_2', content: trimmed.substring(3).trim() });
                continue;
            }
            if (trimmed.startsWith('### ')) {
                blocks.push({ id: uuidv4(), type: 'heading_3', content: trimmed.substring(4).trim() });
                continue;
            }

            // 4. Lists (Bullet)
            if (trimmed.startsWith('- ') || trimmed.startsWith('* ')) {
                blocks.push({ id: uuidv4(), type: 'bullet_list', content: trimmed.substring(2).trim() });
                continue;
            }

            // 5. Lists (Numbered)
            if (/^\d+\.\s/.test(trimmed)) {
                blocks.push({ id: uuidv4(), type: 'numbered_list', content: trimmed.replace(/^\d+\.\s/, '').trim() });
                continue;
            }
            ...
            // 7. Quotes
            if (trimmed.startsWith('> ')) {
                blocks.push({ id: uuidv4(), type: 'quote', content: trimmed.substring(2).trim() });
                continue;
            }
    ...
    static stringify(blocks: Block[]): string {
        return blocks.map(block => {
            switch (block.type) {
                ...
                case 'todo_list': return `- [${block.properties?.checked ? 'x' : ' '}] ${block.content}`;
