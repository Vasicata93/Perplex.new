...
export class BlockService {
     ...
     */
    static insertBlockAfter(page: PageStructure, targetBlockId: string, newBlock: Block): PageStructure {
        ...
        return { ...page, blocks: newBlocks, metadata: { ...page.metadata, lastEditedTime: Date.now() } };
    }

    /**
     * Replaces the content of a specific block.
     */
    static replaceBlock(page: PageStructure, blockId: string, newContent: string): PageStructure {
     ...
     */
    static updateTableCell(page: PageStructure, blockId: string, rowIndex: number, colIndex: number, newValue: string): PageStructure {
        const index = page.blocks.findIndex(b => b.id === blockId);
        if (index === -1) {
            throw new Error(`Block with ID ${blockId} not found.`);
        }
     ...
     */
    static moveBlock(page: PageStructure, blockId: string, targetIndex: number): PageStructure {
        const index = page.blocks.findIndex(b => b.id === blockId);
        if (index === -1) {
            throw new Error(`Block with ID ${blockId} not found.`);
        }
     ...
     */
    static fromMarkdown(markdown: string, title: string = "Untitled"): PageStructure {
