...
export class TavilyService {
   ...
   */
  static async search(query: string, apiKey: string, provider: 'tavily' | 'brave' = 'tavily'): Promise<SearchResponse | null> {
      if (!apiKey) {
          console.warn(`${provider} API Key is missing.`);
          return null;
      }

      const cacheKey = `search_${provider}_${btoa(query)}`;
      
      // 1. Check Cache
      try {
          const cached = await db.get<{ timestamp: number, data: SearchResponse }>(STORES.CACHE, cacheKey);
          if (cached) {
  ...
  private static async searchTavily(query: string, apiKey: string): Promise<SearchResponse | null> {
    try {
  ...
  private static async searchBrave(query: string, apiKey: string): Promise<SearchResponse | null> {
    try {
        ...
        const results: SearchResult[] = rawData.web.results.map((r: any) => {
            ...
            const content = r.description || (r.extra_snippets ? r.extra_snippets.join(' ') : 'No content available.');
