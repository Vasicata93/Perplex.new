...
import { MemoryItem, MemoryCategory, MemoryBufferItem, Project, MemoryContext } from "../types";
...
export class MemoryService {
  ...
  static async addToBuffer(role: 'user' | 'model' | 'tool', content: string): Promise<void> {
    try {
      const buffer = (await db.get<MemoryBufferItem[]>(STORES.BUFFER, BUFFER_KEY)) || [];
  ...
  static async getStructuredContext(prompt: string): Promise<MemoryContext> {
      ...
      const intentLabel = isGeneralIntent ? 'General' : targetCategories[0].toUpperCase();
      ...
      activeProjects.forEach(p => {
           ...
           if (targetCategories.includes('coding') || targetCategories.includes('projects')) {
               workingMemory.push(`Project "${p.title}": ${p.progress} (Next: ${p.nextStep})`);
           } else {
      ...
      const relevantFacts = otherMemories.map(m => {
          let score = 0;
          ...
          if (keywords.some(k => m.content.toLowerCase().includes(k))) score += 3;
  ...
  static async applyConsolidation(updates: any): Promise<void> {
    ...
    const uniqueMemories = currentMemories.filter((m, index, self) => 
        index === self.findIndex((t) => (t.content === m.content && t.category === m.category))
    );

    // 3. Update Projects
    if (updates.project_updates && Array.isArray(updates.project_updates)) {
        updates.project_updates.forEach((update: any) => {
            const existing = currentProjects.find(p => p.title.toLowerCase() === update.title.toLowerCase());
            if (existing) {
    ...
    if (updates.new_skills && Array.isArray(updates.new_skills)) {
         updates.new_skills.forEach((skill: any) => {
             if (!uniqueMemories.find(m => m.category === 'skills' && m.content.includes(skill))) {
                uniqueMemories.push({
                    id: Math.random().toString(36).substr(2, 9),
                    content: skill,
                    category: 'skills',
  ...
  static async addMemory(content: string, category: string = 'other'): Promise<MemoryItem> {
  ...
  private static extractKeywords(text: string): string[] {
    const stopWords = new Set([
        'the', 'is', 'at', 'which', 'on', 'and', 'a', 'an', 'in', 'to', 'of', 'for', 'it', 'this', 'that', 'with', 'as', 'by', 'but', 
        'or', 'not', 'are', 'was', 'were', 'be', 'been', 'being', 'have', 'has', 'had', 'do', 'does', 'did', 'can', 'could', 'will', 
        'would', 'should', 'may', 'might', 'must', 'about', 'from', 'how', 'why', 'what', 'who', 'when', 'where', 'please', 'tell', 'me',
        'remember', 'save', 'my', 'i', 'am'
    ]);
