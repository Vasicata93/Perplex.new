...
import { 
    Search as SearchIcon, MessageSquare, FileText, Calendar, LayoutGrid, 
    ArrowRight, Clock, Tag, MapPin, X, History,
    Sparkles, Command, Filter
} from 'lucide-react';
...
export const SearchView: React.FC<SearchViewProps> = ({
    ...
    const results = useMemo(() => {
        ...
        threads.forEach(t => {
            if (t.title.toLowerCase().includes(q) || t.messages.some(m => m.content.toLowerCase().includes(q))) {
                allResults.push({
                    ...
                    description: t.messages[t.messages.length - 1]?.content.substring(0, 100) + '...',
                    date: t.updatedAt
                });
            }
        });
        ...
        // Search Notes
        notes.forEach(n => {
            if (n.title.toLowerCase().includes(q) || n.content.toLowerCase().includes(q) || n.tags?.some(tag => tag.toLowerCase().includes(q))) {
                allResults.push({
                    id: n.id,
                    type: 'note',
                    title: n.title || 'Untitled Page',
                    description: n.content.substring(0, 100).replace(/[#*`]/g, '') + '...',
                    date: n.updatedAt,
                    icon: n.emoji,
                    tags: n.tags
                });
    ...
    const getIcon = (type: ResultType, icon?: string) => {
        ...
        switch (type) {
            ...
            case 'note': return <FileText size={18} className="text-emerald-400" />;
            case 'event': return <Calendar size={18} className="text-amber-400" />;
            case 'space': return <LayoutGrid size={18} className="text-purple-400" />;
        }
    };
            ...
            <div className="max-w-4xl mx-auto w-full px-6 pt-12 md:pt-20 pb-8 shrink-0">
                <div className="relative group">
                    <div className="absolute inset-y-0 left-6 flex items-center pointer-events-none">
                        <SearchIcon size={24} className="text-pplx-muted group-focus-within:text-pplx-accent transition-colors" />
                    </div>
                    <input
                        ref={inputRef}
                        type="text"
                ...
                <div className="max-w-4xl mx-auto w-full">
                    {!query.trim() ? (
                        <div className="flex flex-col items-center justify-center py-20 text-center animate-fadeIn">
                            ...
                            <h3 className="text-xl font-serif font-bold text-pplx-text mb-2">Search your digital brain</h3>
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0 py-0.5">
                                        ...
                                        <div className="flex flex-wrap gap-2 mt-3">
                                            ...
                                            {result.tags?.map(tag => (
                                                <span key={tag} className="flex items-center gap-1 text-[10px] text-pplx-muted bg-pplx-hover/30 px-2 py-1 rounded-lg">
                                                    <Tag size={10} /> {tag}
                                                </span>
                                            ))}
                                        </div>
                        ...
                        <div className="flex flex-col items-center justify-center py-20 text-center animate-fadeIn">
                            ...
                            <h3 className="text-xl font-serif font-bold text-pplx-text mb-2">No results found</h3>
                            <p className="text-pplx-muted max-w-xs mx-auto text-sm leading-relaxed">
