...
export const MobileDock: React.FC<MobileDockProps> = ({ 
  ...
  const menuItems = [
    ...
    { id: 'shopping', icon: ShoppingCart, label: 'Shop', color: 'bg-orange-500' },
        ...
        <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-[#1a1a1a] w-full max-w-sm rounded-3xl overflow-hidden shadow-2xl border border-white/10">
            <div className="p-4 border-b border-white/10 flex justify-between items-center">
              ...
              <button onClick={() => setShowPagePicker(false)} className="p-2 bg-white/10 rounded-full text-white">
                <X size={18} />
              </button>
            </div>
            <div className="p-3">
            ...
            <div className="max-h-[300px] overflow-y-auto custom-scrollbar p-2 space-y-1">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-white truncate">Favorites</div>
                </div>
                {shortcuts.includes('favorites') && <div className="w-2 h-2 rounded-full bg-[#e8dcc4]" />}
              </button>
              <button
              ...
              >
                <span className="text-xl">📅</span>
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-white truncate">Calendar</div>
                </div>
                {shortcuts.includes('calendar') && <div className="w-2 h-2 rounded-full bg-[#e8dcc4]" />}
              </button>
              
              <div className="h-px bg-white/10 my-2" />

              {notes.filter(n => n.title.toLowerCase().includes(pickerSearch.toLowerCase())).map(note => (
                <button
                  key={note.id}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-white/40 truncate">{new Date(note.updatedAt).toLocaleDateString()}</div>
                  </div>
                  {shortcuts.includes(note.id) && <div className="w-2 h-2 rounded-full bg-[#e8dcc4]" />}
                </button>
              ))}
            </div>
          </div>
