...
export const SpaceFilesModal: React.FC<SpaceFilesModalProps> = ({ isOpen, onClose, space, onUpdateSpace }) => {
      ...
      <div className="bg-pplx-card w-full max-w-lg rounded-2xl shadow-2xl border border-pplx-border overflow-hidden flex flex-col max-h-[80vh]">
        ...
        <div className="flex items-center justify-between p-4 border-b border-pplx-border bg-pplx-secondary/50">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-pplx-text">Manage Knowledge</h2>
          </div>
          <button onClick={onClose} className="p-1.5 text-pplx-muted hover:text-pplx-text hover:bg-pplx-hover rounded-full transition-colors">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
          ...
          <div className="space-y-2">
            {files.length > 0 ? (
              files.map((file, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 bg-pplx-secondary/30 border border-pplx-border rounded-xl hover:border-pplx-border/80 transition-colors group">
              ...
              <div className="flex flex-col items-center justify-center py-12 text-center border-2 border-dashed border-pplx-border/50 rounded-xl bg-pplx-secondary/10">
                ...
                <p className="text-sm font-medium text-pplx-text">No knowledge files yet</p>
                <p className="text-xs text-pplx-muted mt-1 max-w-[200px]">Upload documents to give context to your workspace AI.</p>
              </div>
            )}
          </div>
        </div>
