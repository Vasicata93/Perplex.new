...
const WeatherWidget = ({ selectedDate }: { selectedDate: Date }) => {
  const [weather, setWeather] = useState<{ temp: number, condition: string, code: number, wind: number } | null>(null);
  const [location, setLocation] = useState({ name: 'București', lat: 44.4268, lon: 26.1025 });
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const [searchCity, setSearchCity] = useState('');
  const [searchResults, setSearchResults] = useState<any[]>([]);
  const [isSearching, setIsSearching] = useState(false);

  const fetchWeather = async (lat: number, lon: number, name?: string, targetDate?: Date) => {
    try {
  ...
  const getWeatherIcon = (code: number, size = 20) => {
    ...
    if (code <= 48) return <CloudFog {...iconProps} className="text-pplx-muted" />;
          ...
          <div className="absolute top-full left-0 mt-3 w-60 bg-pplx-card border border-pplx-border rounded-[20px] shadow-2xl z-[121] overflow-hidden animate-in fade-in zoom-in-95 duration-300">
                ...
                <div className="mt-2 space-y-1 max-h-40 overflow-y-auto custom-scrollbar border-t border-pplx-border pt-2">
                  ...
                  {searchResults.map((city, idx) => (
                    <button
                      key={`${city.id}-${idx}`}
                      onClick={() => {
                        fetchWeather(city.latitude, city.longitude, city.name, selectedDate);
                        setIsMenuOpen(false);
                        setSearchCity('');
                        setSearchResults([]);
                      }}
            ...
            <div className="max-h-48 overflow-y-auto p-1.5 no-scrollbar">
              {CITIES.map((city) => (
                ...
                >
                  <span>{city.name}</span>
                  {location.name === city.name && <div className="w-1 h-1 rounded-full bg-blue-400 shadow-[0_0_8px_rgba(96,165,250,0.8)]" />}
                </button>
              ))}
            </div>
          </div>
...
export const CalendarView: React.FC<CalendarViewProps> = ({
  ...
  const [holidays, setHolidays] = useState<Holiday[]>(() => getHolidays(new Date().getFullYear()));
  ...
  const handleSaveEvent = () => {
    if (!newEventData.title || !newEventData.startDate || !newEventData.endDate) return;
                  ...
                  upcoming.map(event => (
                      ...
                      >
                          ...
                          <div className="flex-1 min-w-0">
                              <div className="text-sm font-bold text-pplx-text truncate group-hover:text-blue-400 transition-colors">{event.title}</div>
  ...
  const renderMonthView = () => {
    ...
    return (
      <div className="grid grid-cols-7 gap-px bg-pplx-border h-full border-x border-b border-pplx-border rounded-b-3xl md:rounded-2xl overflow-hidden shadow-2xl md:bg-pplx-border md:border">
        ...
        {/* Days */}
        {[...blanks, ...dayNumbers].map((day, idx) => {
          if (!day) return <div key={`blank-${idx}`} className="bg-pplx-input min-h-[40px] md:min-h-[80px]" />;
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto no-scrollbar flex flex-col gap-0.5">
                {dayEvents.map(event => (
                  ...
                  >
                    {event.allDay ? '' : <span className="hidden md:inline opacity-75 mr-1 text-[7px]">{new Date(event.startDate).getHours()}h</span>}
                    {event.title}
                  </div>
                ))}
              </div>
      ...
      <div className="flex flex-col h-full overflow-hidden border border-pplx-border rounded-2xl bg-pplx-card shadow-2xl">
        ...
        <div className="grid grid-cols-8 border-b border-pplx-border bg-pplx-hover/5">
          ...
          {weekDays.map((day, i) => {
             ...
             const hasPublicHoliday = dayHolidays.some(h => h.isPublic);
             return (
              <div key={i} className={`p-3 text-center border-r border-pplx-border last:border-r-0 ${isToday ? 'bg-blue-500/5' : hasPublicHoliday ? 'bg-red-500/5' : ''}`}>
                <div className={`text-[10px] uppercase font-black tracking-[0.2em] mb-1 ${hasPublicHoliday ? 'text-red-400' : 'text-pplx-muted'}`}>{DAYS[day.getDay()]}</div>
                <div className={`text-lg font-light ${isToday ? 'text-blue-500 font-bold' : hasPublicHoliday ? 'text-red-400' : 'text-pplx-text'}`}>{day.getDate()}</div>
    ...
    return (
      <div className="flex flex-col h-full overflow-hidden border border-pplx-border rounded-2xl bg-pplx-card shadow-2xl">
        ...
        <div className="border-b border-pplx-border p-6 text-center bg-pplx-hover/5">
          <div className={`text-[10px] uppercase font-black tracking-[0.3em] mb-2 ${hasPublicHoliday ? 'text-red-400' : 'text-pplx-muted'}`}>{DAYS[day.getDay()]}</div>
          <div className={`text-4xl font-serif font-bold ${isToday ? 'text-blue-500' : hasPublicHoliday ? 'text-red-400' : 'text-pplx-text'}`}>{day.getDate()} {MONTHS[day.getMonth()]}</div>
          {dayHolidays.map((h, hIdx) => (
            <div 
              key={hIdx}
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 pt-1">
                  <h3 className="font-serif text-lg md:text-xl font-bold text-pplx-text group-hover:text-blue-400 transition-colors">{event.title}</h3>
                  ...
                  {event.description && (
                    <p className="mt-3 text-xs md:text-sm text-pplx-muted/80 leading-relaxed line-clamp-2">{event.description}</p>
                  )}
                </div>
              </div>
            );
            ...
            return (
              ...
              >
                <div className={`flex flex-col items-center min-w-[60px] md:min-w-[70px] border rounded-xl p-2 shadow-lg ${holiday.isPublic ? 'bg-red-500/10 border-red-500/20' : 'bg-emerald-500/10 border-emerald-500/20'}`}>
                  ...
                  <span className={`text-2xl md:text-3xl font-light my-1 ${holiday.isPublic ? 'text-red-500' : 'text-emerald-500'}`}>{date.getDate()}</span>
                ...
                <div className="flex-1 pt-1">
                  ...
                  <div className="flex flex-wrap items-center gap-3 mt-2 text-[11px] md:text-xs text-pplx-muted">
                    ...
                    <span className="text-pplx-muted/60 italic">System Event</span>
                  </div>
                </div>
              </div>
            );
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 h-full overflow-y-auto no-scrollbar p-4 bg-pplx-primary">
        {months.map(month => {
          ...
          return (
            <div key={month} className="bg-pplx-card p-4 rounded-2xl border border-pplx-border hover:border-blue-500/30 transition-all cursor-pointer shadow-sm hover:shadow-md" onClick={() => {
              setCurrentDate(date);
              setViewMode('month');
            }}>
              <h3 className="text-xs font-bold text-pplx-text mb-3 uppercase tracking-widest border-b border-pplx-border pb-2">{MONTHS[month]}</h3>
              <div className="grid grid-cols-7 gap-1 text-center">
                {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((d, i) => (
                  <div key={i} className="text-[8px] text-pplx-muted font-bold mb-1">{d}</div>
                ))}
                {[...blanks, ...dayNumbers].map((day, idx) => {
                  const isToday = day && isSameDay(new Date(year, month, day), new Date());
                  return (
                    <div key={idx} className={`text-[9px] h-5 flex items-center justify-center rounded-full transition-colors ${day ? 'text-pplx-text' : ''} ${isToday ? 'bg-blue-500 text-white font-bold' : 'hover:bg-pplx-hover/30'}`}>
                      {day || ''}
                    </div>
                  );
                })}
    ...
    <div className="flex flex-col h-full w-full bg-pplx-primary p-0 md:p-2 gap-0 overflow-hidden">
      ...
      {todayHoliday && (
        ...
        >
          <div className="flex items-center gap-2">
            <Info size={14} />
            <span>Astăzi: {todayHoliday.name} ({todayHoliday.country}) {todayHoliday.isPublic ? '— Zi Nelucrătoare' : ''}</span>
          </div>
          <div className="flex items-center gap-2 opacity-60">
            <span className="text-[9px] uppercase tracking-widest">Sărbătoare de Sistem</span>
          </div>
        </div>
      )}
        ...
        <div className="flex flex-col md:flex-row items-center gap-3 w-full md:w-auto justify-between md:justify-start">
            ...
            <div className="flex md:hidden items-center justify-between w-full">
                ...
                <div className="flex items-center gap-3">
                    <button onClick={handlePrev} className="p-2 hover:bg-pplx-hover rounded-xl text-pplx-muted active:scale-90 transition-all border border-transparent hover:border-pplx-border">
                        <ChevronLeft size={20} />
                    </button>
                    ...
                    <button onClick={handleNext} className="p-2 hover:bg-pplx-hover rounded-xl text-pplx-muted active:scale-90 transition-all border border-transparent hover:border-pplx-border">
            ...
            <div className="hidden md:flex items-center gap-1 shrink-0 bg-pplx-card border border-pplx-border rounded-2xl p-1 shadow-sm">
                <button onClick={handlePrev} className="p-1.5 hover:bg-pplx-hover rounded-xl text-pplx-muted hover:text-pplx-text transition-all"><ChevronLeft size={18} /></button>
                ...
                <button onClick={handleNext} className="p-1.5 hover:bg-pplx-hover rounded-xl text-pplx-muted hover:text-pplx-text transition-all"><ChevronRight size={18} /></button>
            </div>
        </div>

        {/* Center: Search Bar (Desktop) */}
        <div className="hidden md:block flex-1 max-w-md w-full relative group">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-pplx-muted group-focus-within:text-blue-500 transition-colors" size={14} />
            <input 
                type="text"
                value={searchQuery}
                ...
                {isViewMenuOpen && (
                    <div className="absolute right-0 top-full mt-2 w-36 bg-pplx-card border border-pplx-border rounded-2xl shadow-2xl z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-200 p-1">
                        {(['year', 'month', 'week', 'day'] as ViewMode[]).map((mode) => (
                            ...
                            >
                                {mode}
                                {viewMode === mode && <div className="w-1.5 h-1.5 rounded-full bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.5)]" />}
                            </button>
                        ))}
                    </div>
                )}
                ...
                <div className="p-4 border-b border-pplx-border bg-pplx-card/50 backdrop-blur-md sticky top-0">
                    <div className="flex items-center gap-3">
                        <div className="flex-1 relative">
                            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-pplx-muted" size={18} />
                            <input 
                    ...
                    {filteredEvents.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 gap-3">
                            {filteredEvents.map(event => (
                                ...
                                >
                                    <div className={`w-1.5 h-12 rounded-full ${event.color || 'bg-blue-600'} shrink-0`} />
                                    <div className="flex-1 min-w-0">
                                        <div className="font-bold text-pplx-text text-lg truncate">{event.title}</div>
              ...
              <div className="flex flex-col">
                <div className="flex items-center gap-2 mb-2">
                  <span className="px-2 py-0.5 bg-pplx-hover/20 rounded-full text-[9px] font-black uppercase tracking-widest text-white shadow-sm border border-pplx-border/20">
                    {selectedHoliday.country === 'BOTH' ? 'RO & DE' : selectedHoliday.country === 'INT' ? 'INT' : selectedHoliday.country}
                  </span>
                </div>
            ...
            <div className="p-8 space-y-8 max-h-[60vh] overflow-y-auto no-scrollbar">
              <div className="flex items-center gap-5">
                <div className="flex flex-col items-center justify-center w-14 h-14 bg-pplx-hover/20 border border-pplx-border rounded-2xl shadow-sm">
                  <span className="text-[9px] font-black text-pplx-muted uppercase tracking-tighter">{new Date(selectedHoliday.date).toLocaleDateString('ro-RO', { month: 'short' })}</span>
                  <span className="text-xl font-light text-pplx-text">{new Date(selectedHoliday.date).getDate()}</span>
                </div>
                <div className="flex-1">
                  <div className="text-[10px] font-black text-pplx-muted uppercase tracking-[0.2em] mb-1">Data</div>
                  <div className="text-pplx-text text-base font-medium">
                    {new Date(selectedHoliday.date).toLocaleDateString('ro-RO', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                  </div>
                </div>
              </div>

              <div className="space-y-3">
                <div className="text-[10px] font-black text-pplx-muted uppercase tracking-[0.2em]">Descriere</div>
                <p className="text-pplx-muted leading-relaxed text-sm font-light italic bg-pplx-hover/5 p-4 rounded-2xl border border-pplx-border/50">
                  {selectedHoliday.description || "Informații detaliate despre această sărbătoare vor fi disponibile în curând."}
                </p>
              </div>
              ...
              <div className="pt-6 border-t border-pplx-border flex justify-between items-center">
                <div className="flex items-center gap-3 px-3 py-1.5 bg-pplx-hover/10 rounded-full border border-pplx-border/50">
                  <div className={`w-2 h-2 rounded-full shadow-sm ${selectedHoliday.type === 'religious' ? 'bg-emerald-500 shadow-emerald-500/40' : selectedHoliday.type === 'national' ? 'bg-blue-500 shadow-blue-500/40' : 'bg-purple-500 shadow-purple-500/40'}`} />
                  <span className="text-[10px] font-bold text-pplx-muted uppercase tracking-wider">{selectedHoliday.type === 'religious' ? 'Religios' : selectedHoliday.type === 'national' ? 'Național' : 'Cultural'}</span>
                </div>
                <button 
                  onClick={() => setIsHolidayModalOpen(false)}
        ...
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-xl flex items-center justify-center p-2 md:p-4">
            <div className="bg-pplx-card border border-pplx-border rounded-[32px] shadow-2xl w-full max-w-md max-h-[95vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-300">
                ...
                <div className="flex-1 overflow-y-auto no-scrollbar p-6 md:p-8 space-y-6 md:space-y-8">
                    <div>
                        <label className="block text-[10px] font-black text-pplx-muted uppercase mb-3 tracking-[0.2em]">Title</label>
                        <input 
                            type="text" 
                    ...
                    <div className="space-y-6">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div className="space-y-3">
                                <label className="block text-[10px] font-black text-pplx-muted uppercase tracking-[0.2em]">Start Date & Time</label>
                            ...
                            <div className="space-y-3">
                                <label className="block text-[10px] font-black text-pplx-muted uppercase tracking-[0.2em]">End Date & Time</label>
                                <div className="flex gap-2">
                                    <input 
                                        type="date" 
                    ...
                    <div className="flex items-center gap-4 bg-pplx-hover/5 p-4 md:p-5 rounded-2xl border border-pplx-border/50">
                        ...
                        />
                        <label htmlFor="allDay" className="text-sm font-black text-pplx-text/80 cursor-pointer select-none uppercase tracking-wider">All Day Event</label>
                    </div>

                    <div>
                        <label className="block text-[10px] font-black text-pplx-muted uppercase mb-3 tracking-[0.2em]">Location</label>
                        <div className="relative group">
                            <MapPin size={18} className="absolute left-4 top-3.5 md:top-4 text-pplx-muted/40 group-focus-within:text-blue-500 transition-colors" />
                            <input 
                                type="text" 
                    ...
                    <div>
                        <label className="block text-[10px] font-black text-pplx-muted uppercase mb-3 tracking-[0.2em]">Description</label>
                        <textarea 
                    ...
                    <div>
                        <label className="block text-[10px] font-black text-pplx-muted uppercase mb-4 tracking-[0.2em]">Color</label>
