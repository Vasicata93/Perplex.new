...
import { X, Plus, Trash2, FileText, Upload, Save, MoreHorizontal } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 md:p-0">
      <div className="bg-pplx-sidebar w-full max-w-4xl rounded-xl shadow-2xl border border-pplx-border overflow-hidden flex flex-col md:flex-row h-full max-h-[90vh] md:h-[85vh]">
                ...
                {spaces.map(space => (
                    <div key={space.id} 
                        className={`group w-full flex items-center justify-between px-3 py-3 rounded-lg text-sm transition-colors cursor-pointer ${
                            editingSpace?.id === space.id ? 'bg-pplx-hover text-pplx-text' : 'text-pplx-muted hover:bg-pplx-secondary hover:text-pplx-text'
                        }`}
                    ...
                    >
                        <div className="flex items-center space-x-3 truncate">
                            ...
                            <div className="flex flex-col text-left truncate">
                                ...
                                <span className="text-[10px] opacity-60 truncate">{space.description || 'No description'}</span>
                            </div>
                        </div>
                        <button 
        ...
        <div className={`flex-1 bg-pplx-sidebar flex-col h-full relative ${!editingSpace ? 'hidden md:flex' : 'flex'}`}>
            <button onClick={() => {
            ...
            {editingSpace ? (
                <div className="flex flex-col h-full p-4 md:p-8 overflow-y-auto custom-scrollbar">
                    <div className="mb-6">
                        <label className="block text-xs font-semibold text-pplx-muted uppercase tracking-wider mb-2">Space Icon & Name</label>
                        <div className="flex gap-3">
                            <input 
                                ...
                                onChange={(e) => setEditingSpace({...editingSpace, emoji: e.target.value})}
                                maxLength={2}
                            />
                    ...
                    <div className="mb-8">
                        <div className="flex justify-between items-center mb-2">
                            <label className="block text-xs font-semibold text-pplx-muted uppercase tracking-wider">Knowledge Base</label>
                            <button 
                        ...
                        <div className="space-y-2">
                            {editingSpace.files?.map((file, idx) => (
                                <div key={idx} className="flex items-center justify-between p-3 bg-pplx-input border border-pplx-border rounded-lg">
                                    <div className="flex items-center gap-3 overflow-hidden">
                                        <div className="bg-pplx-hover p-2 rounded">
                                            <FileText size={16} className="text-pplx-muted" />
                                        </div>
                                        <div className="flex flex-col truncate">
                                            <span className="text-sm font-medium text-pplx-text truncate">{file.name}</span>
                                            <span className="text-[10px] text-pplx-muted uppercase">{file.mimeType.split('/')[1] || 'FILE'}</span>
                                        </div>
                                    </div>
                                    <button onClick={() => removeFile(idx)} className="text-pplx-muted hover:text-red-400 p-2">
                                        <X size={16} />
                                    </button>
                                </div>
                            ))}
                    ...
                    <div 
                        ...
                        style={{
                            paddingBottom: document.body.classList.contains('dock-active') && window.innerWidth < 768 
                ...
                <div className="flex flex-col items-center justify-center h-full text-pplx-muted">
                    ...
                    <h3 className="text-lg font-medium text-pplx-text mb-2">Select a Space to edit</h3>
                    <p className="text-sm">Or create a new one to organize your knowledge.</p>
                </div>
            )}
        </div>
      </div>
