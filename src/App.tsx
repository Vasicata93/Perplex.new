...
:::`;
  ...
  const handleSendMessage = async (
    ...
    if (isAgentMode) {
      ...
      return; // Skip the standard LLM flow
    }
    ...
    if (focusModes.includes(FocusMode.LIBRARY)) {
      const hasWebSearch =
        focusModes.includes(FocusMode.WEB_SEARCH) ||
    ...
    try {
      ...
      if (modelId) {
        if (modelId.startsWith("gemini")) {
          ...
          const found = settings.localModels.find((m) => m.id === modelId);
          if (found) {
